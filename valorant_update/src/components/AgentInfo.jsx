  ...
  return (
    <>
      ...
      <div className='sm:hidden h-screen w-screen bg-valorant-black flex flex-col justify-center items-start px-5 gap-5'>
        ...
        <p className='font-sans text-[19.2px] text-valorant-white leading-tight tracking-wide'>{agent?.data.description}</p>
      ...
      <div className='hidden h-screen w-screen bg-valorant-black sm:flex flex-col justify-center items-start px-20 gap-5 relative overflow-hidden'>
        <div className='flex flex-col items-start gap-1'>
          <button onClick={handleGoBack} className='text-valorant-red uppercase font-sans font-bold z-10 hover:text-valorant-white mt-10 2xl:mt-0'>
            back
          </button>
          <div className='font-tungsten font-bold uppercase tracking-normal leading-[6rem] text-valorant-white text-[160px] -ml-2 2xl:mt-20 mt-10'>
            {agent?.data.displayName}
          </div>
        </div>

        <div className='font-Playfair italic text-valorant-red text-[64px] leading-[2.7rem]'>{agent?.data.role.displayName}</div>

        <p className='font-sans text-[19.2px] text-valorant-white leading-tight tracking-wider w-[390px] mb-16'>{agent?.data.description}</p>
        ...
        <img
          className='absolute scale-[.50] left-96 -bottom-72 2xl:scale-50 2xl:-bottom-[425px] z-10'
          src={agent?.data.fullPortrait}
          alt={agent?.data.displayName}
        />
        <img
          className='absolute scale-[.80] right-0 -top-10 2xl:scale-100 2xl:top-10 2xl:right-56'
